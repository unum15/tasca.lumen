...
class ServiceOrderPriorityControllerTest extends TestCase
{
    /**
    ...
    public function testIndex()
    {
        ...
        $response0 = $this->post('/service_order_priority',$items[0]);
        $response0->seeStatusCode(200);                
        $response1 = $this->post('/service_order_priority',$items[1]);
        $response1->seeStatusCode(200);                
        $response = $this->get('/service_order_priorities');
        $response->seeStatusCode(200);
        $response->seeJson($items[0]);
        $response->seeJson($items[1]);
        $dbitems = ServiceOrderPriority::all();
    ...
    public function testCreate()
    {
        ...
        $response = $this->post('/service_order_priority',$item);
        $response->seeStatusCode(200);                
        $response->seeJson($item);
        $response_array = json_decode($response->response->getContent());
        $dbitem = ServiceOrderPriority::find($response_array->id);
    ...
    {
        $item = ['name' => "a'; DROP TABLE service_order_prioritys CASCADE; --", 'notes' => "a'; DROP TABLE activity_levels CASCADE; --"];
        $response = $this->post('/service_order_priority',$item);
        $response->seeStatusCode(200);                
        $response->seeJson($item);
        $response_array = json_decode($response->response->getContent());        
    ...
    public function testRead()
    {
        ...
        $response->seeStatusCode(200);
        $response_array = json_decode($response->response->getContent());
        $response = $this->get('/service_order_priority/' . $response_array->id);
        $response->seeStatusCode(200);
        $response->seeJson($item);        
        $dbitem = ServiceOrderPriority::find($response_array->id);
        $response->seeJsonEquals($dbitem->toArray());
    ...
    {        
        $response = $this->get('/service_order_priority/a');
    ...
    public function testCreateDoubleDefault()
    {
        ...
        $response = $this->post('/service_order_priority',$items[0]);
        $response->seeStatusCode(200);
        $response_array = json_decode($response->response->getContent());        
        $response = $this->post('/service_order_priority',$items[1]);
        $response->seeStatusCode(200);                
        $dbitem = ServiceOrderPriority::find($response_array->id);
    ...
    public function testUpdate()
    {
        ...
        $response_array = json_decode($response->response->getContent());
        $patch = ['name' => 'Test 2'];
        $response = $this->patch('/service_order_priority/' . $response_array->id, $patch);
        $response->seeStatusCode(200);
    ...
    public function testDelete()
    {
        ...
        $response->seeStatusCode(200);
        $response_array = json_decode($response->response->getContent());
        $response = $this->delete('/service_order_priority/' . $response_array->id);
