...
class ProjectControllerTest extends TestCase
{
    ...
    public function testCreate()
    {
        ...
        $response = $this->actingAs($this->contact)->post('/project', $item);
        $response->seeStatusCode(200);
    ...
    {        
        $response = $this->actingAs($this->contact)->get('/project/' . $this->item->id);
        $response->seeStatusCode(200);
        $response->seeJson($this->item->toArray());        
        $dbitem = Project::find($this->item->id);
    ...
    {        
        $response = $this->actingAs($this->contact)->get('/project/a');
    ...
    {        
        ...
        $response = $this->actingAs($this->contact)->patch('/project/' . $this->item->id, $patch);
        $response->seeStatusCode(200);
    ...
    {        
        $response = $this->actingAs($this->contact)->delete('/project/' . $this->item->id);
