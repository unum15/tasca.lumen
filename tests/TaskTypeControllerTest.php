...
class TaskTypeControllerTest extends TestCase
{
    ...
    public function testIndex()
    {
        ...
        $response0 = $this->post('/task_type',$items[0]);
        $response0->seeStatusCode(200);                
        $response1 = $this->post('/task_type',$items[1]);
        $response1->seeStatusCode(200);                
        $response = $this->get('/task_types');
        $response->seeStatusCode(200);
        $response->seeJson($items[0]);
        $response->seeJson($items[1]);
        $dbitems = TaskType::all();
    ...
    {
        $item = ['name' => "a'; DROP TABLE task_types CASCADE; --", 'notes' => "a'; DROP TABLE activity_levels CASCADE; --"];
        $response = $this->post('/task_type',$item);
        $response->seeStatusCode(200);                
        $response->seeJson($item);
        $response_array = json_decode($response->response->getContent());        
    ...
    public function testRead()
    {
        ...
        $response->seeStatusCode(200);
        $response_array = json_decode($response->response->getContent());
        $response = $this->get('/task_type/' . $response_array->id);
        $response->seeStatusCode(200);
        $response->seeJson($item);        
        $dbitem = TaskType::find($response_array->id);
        $response->seeJsonEquals($dbitem->toArray());
    ...
    public function testCreateDoubleDefault()
    {
        ...
        $response = $this->post('/task_type',$items[0]);
        $response->seeStatusCode(200);
        $response_array = json_decode($response->response->getContent());        
        $response = $this->post('/task_type',$items[1]);
        $response->seeStatusCode(200);                
        $dbitem = TaskType::find($response_array->id);
    ...
    public function testUpdate()
    {
        ...
        $response_array = json_decode($response->response->getContent());
        $patch = ['name' => 'Test 2'];
        $response = $this->patch('/task_type/' . $response_array->id, $patch);
        $response->seeStatusCode(200);
    ...
    public function testDelete()
    {
        ...
        $response->seeStatusCode(200);
        $response_array = json_decode($response->response->getContent());
        $response = $this->delete('/task_type/' . $response_array->id);
